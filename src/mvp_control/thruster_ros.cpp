...
double ThrusterROS::normalize_angle(double angle) {
    // Range of the input angle
    double input_min = -3.14, input_max = 3.14;

    // Desired output range
    double output_min = -1, output_max = 1;

    // Calculate normalized angle using linear transformation
    double normalized_angle = (angle - input_min) / (input_max - input_min) 
                * (output_max - output_min) + output_min;
                
    return std::max(std::min(normalized_angle, output_max), output_min);
}