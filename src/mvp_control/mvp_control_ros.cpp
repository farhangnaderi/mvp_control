...
// Definition of f_getJointLimits
    ...
    for (const auto& t : m_thrusters) {
        ...
        */
        ...
        if (!f_getJointLimits(m_model, joint_name, angle_min, angle_max)) {
            ROS_WARN("Failed to get joint limits for joint '%s'. Setting to zero.", joint_name.c_str());
            angle_min = 0.0;
            angle_max = 0.0;
        } else {
            ROS_INFO("Joint '%s': angle_min = %f, angle_max = %f", joint_name.c_str(), angle_min, angle_max);
        }
