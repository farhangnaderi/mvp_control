...
bool MvpControl::f_optimize_thrust(Eigen::VectorXd *t, Eigen::VectorXd u) {
    ...
    const double gamma_lower = -3.14;
    const double gamma_upper = 3.14;
    ...
    // Debugging the A_sparse matrix
    ROS_INFO_STREAM("A_sparse rows: " << A_sparse.rows() << ", cols: " << A_sparse.cols());
    for (int k = 0; k < A_sparse.outerSize(); ++k) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(A_sparse, k); it; ++it) {
            ROS_INFO_STREAM("A(" << it.row() << "," << it.col() << ") = " << it.value());
        }
    }
